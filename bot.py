...
async def choose_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data_store[user_id] = {"task": text, "data": []}
    ...
    await update.message.reply_text(f"{get_task_description(text)}\n\nВводи данные по одному числу. Когда закончишь, напиши 'готово'.")
...
async def collect_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() == "готово":
        ...
        if task == "1":
            ...
            await update.message.reply_text(f"Δr = {dx}i + {dy}j + {dz}k\n|Δr| = {mod:.4f}")
        elif task == "2":
            if len(data) < 1:
            ...
            if tip == 1 and len(data) >= 4:
                ...
                await update.message.reply_text(f"d²r/dz² = {n*(n+1)*A}/z^{n+2} - {B}/z²")
            elif tip == 2:
                await update.message.reply_text("d²r/dφ² = 2(1+tg²φ)(1+3tg²φ)")
            else:
                await update.message.reply_text("Ошибка: для типа 1 нужно A, n, B")
        elif task == "3":
            if len(data) != 2:
                await update.message.reply_text("Введи n и C")
                return WAITING_FOR_DATA
            n, C = data
            await update.message.reply_text(f"∂f/∂y = -{n}·(x·z)/y^{n+1} - 1/(2x√y)")
        elif task == "4":
            if len(data) != 2:
            ...
            await update.message.reply_text(f"∫₀¹ ({A}·e^x + {B}·x²) dx = {result:.4f}")
        elif task == "5":
            if len(data) != 3:
            ...
            await update.message.reply_text(f"|d| = √{d2:.4f} = {math.sqrt(d2):.4f}")
        elif task == "6":
            if len(data) != 2:
                await update.message.reply_text("Введи |h| и угол с OX")
                return WAITING_FOR_DATA
            h, ang = data
            hx = h * math.cos(math.radians(ang))
            hy = h * math.sin(math.radians(ang))
            await update.message.reply_text(f"h_x = {hx:.4f}, h_y = {hy:.4f}")
        elif task == "7":
            if len(data) != 3:
                await update.message.reply_text("Введи |a|, |b| и угол")
                return WAITING_FOR_DATA
            ...
            res = a**2 + 4*b**2 + 4*ab
            await update.message.reply_text(f"(a+2b)² = {res:.4f}")
        elif task == "8":
            if len(data) != 2:
                await update.message.reply_text("Введи |a| и |b|")
                return WAITING_FOR_DATA
            a, b = data
            await update.message.reply_text(f"Проекция b на a = {b}·cos90° = 0")
        else:
